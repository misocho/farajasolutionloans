...
- **Maps links (2026-08-11, C7):** client registration form takes optional Google Maps share links for residential (`residential_maps_link`) and business (`business_maps_link`) locations. Frontend `Client` type + payload carry them (snake_case from `features/clients/api.ts`); file-local `MapsLinkPreview` component validates google-maps URLs and embeds a keyless iframe (`https://maps.google.com/maps?q=<link>&output=embed`) for `maps.google.com`/`www.google.com/maps` hosts — short links (`goo.gl/maps`, `maps.app.goo.gl`) render link-only. Client detail drawer shows clickable "Open in Google Maps" rows for both.
