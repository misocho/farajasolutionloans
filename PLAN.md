...
- [x] U2 User details drawer — `GET /admin/users/{id}` (`UserAdminDetailResponse`: profile + roles + branches + status + last login + created_at + phone/id_no + full permission list via `get_user_permissions`) + "View" action → slide-in drawer in the users table (matches clients/loans drawer pattern; error + retry states). Verified: DIR + SYS 200, LO 403, bogus UUID 404
- [x] U3 Search deep-links to the record — topbar result click → `/clients?client=<id>` / `/loans?loan=<id>`; clients page opens the C3 detail drawer from the param (loader while fetching), loans page opens the LoanDrawer; `/loans?apply=true` (sidebar "Apply for Loan") now opens the new-loan form; params cleared via `history.replaceState` after handling
...
- [ ] Location fields — Google Maps link input + preview (currently plain text) — form inputs **removed 2026-08-11** (owner); only the detail-drawer map links remain (see C7)
