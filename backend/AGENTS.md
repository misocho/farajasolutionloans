...
2. **Auth + permission pattern (canonical).** Every endpoint: `current_user=Depends(get_current_user)` then `_require_permission(db, current_user, "perm.name")` — the signature is `(db, user, perm)`.
   ...
   - Director/System Admin gates (admin.py) use role-name checks (`check_is_director`) — keep the two patterns separate: permission-based for features, role-based only for admin console.
